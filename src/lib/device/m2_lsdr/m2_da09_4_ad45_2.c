...
struct dev_m2_d09_4_ad45_2 {
    device_t base;
    usdr_vfs_obj_constant_t vfs_const_objs[SIZEOF_ARRAY(s_params_m2_da09_4_ad45_2_rev000)];
...
int dev_m2_d09_4_ad45_2_rate_set(pdevice_t ud, pusdr_vfs_obj_t obj, uint64_t value)
{
    ...
    if (res)
        return res;
    res = dev_gpo_set(d->base.dev, IGPO_BANK_CFG_ADC, 0x00);
    if (res)
        return res;

    res = ads42lbx9_set_normal(dev, 0, SPI_ADS42LBX9_0, true, false);
    if (res) {
        return res;
    }
    res = ads42lbx9_set_normal(dev, 0, SPI_ADS42LBX9_1, false, false);
    if (res) {
        return res;
    }
...
int dev_m2_d09_4_ad45_2_pwren_set(pdevice_t ud, pusdr_vfs_obj_t obj, uint64_t value)
{
 ...
 /*   int res;
    ...
    res = tps6594_vout_ctrl(d->base.dev, 0, 1, TPS6594_LDO4, (value) ? true : false);
    if (res)
        return res;
...
static int dev_m2_d09_4_ad45_2_debug_rxtime_get(pdevice_t ud, pusdr_vfs_obj_t obj, uint64_t* ovalue)
{
    ...
    //res = lowlevel_reg_rd32(d->base.dev, 0, 0, &tmp);
    //if (res)
    //    return res;
...
int usdr_device_m2_d09_4_ad45_2_create(lldev_t dev, device_id_t devid)
{
    ...
    uid += SIZEOF_ARRAY(s_params_m2_da09_4_ad45_2_rev000);
