...
static float* out[3] = {NULL, NULL, NULL};
...
static void setup()
{
    ...
    res = res ? res : posix_memalign((void**)&out1,        ALIGN_BYTES, sizeof(float) * SPEED_WORD_COUNT/3);
    res = res ? res : posix_memalign((void**)&out1_etalon, ALIGN_BYTES, sizeof(float) * SPEED_WORD_COUNT/3);
    res = res ? res : posix_memalign((void**)&out2,        ALIGN_BYTES, sizeof(float) * SPEED_WORD_COUNT/3);
    res = res ? res : posix_memalign((void**)&out2_etalon, ALIGN_BYTES, sizeof(float) * SPEED_WORD_COUNT/3);
    res = res ? res : posix_memalign((void**)&out3,        ALIGN_BYTES, sizeof(float) * SPEED_WORD_COUNT/3);
    res = res ? res : posix_memalign((void**)&out3_etalon, ALIGN_BYTES, sizeof(float) * SPEED_WORD_COUNT/3);
...
{
    return generic_get_fn(o, log, conv_get_ci16_3cf32_c, &last_fn_name);
}
...
static void print_data(const char* header)
{
    ...
    for(unsigned i = 0; i < 24; ++i)
    {
        ...
        fprintf(stderr, "%.4f ", (double)in[i] / 32767.f);
    }
...
START_TEST(conv_ci16_3cf32_check_simd)
{
...
START_TEST(conv_ci16_3cf32_speed)
{
    generic_opts_t opt = max_opt;
    conv_function_t fn = NULL;
    const void* pin = (const void*)in;
...
Suite * conv_ci16_3cf32_suite(void)
{
    max_opt = cpu_vcap_get();

    Suite* s = suite_create("conv_ci16_3cf32");

    ADD_REGRESS_TEST(s, conv_ci16_3cf32_check_simd);
    ADD_PERF_LOOP_TEST(s, conv_ci16_3cf32_speed, 60, 0, 3);
