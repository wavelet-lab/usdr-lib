...
static float* in[3] = {NULL, NULL, NULL};
...
{
    int res = 0;
    res = res ? res : posix_memalign((void**)&in_0,       ALIGN_BYTES, WORD_COUNT * sizeof(float) / 3);
    res = res ? res : posix_memalign((void**)&in_1,       ALIGN_BYTES, WORD_COUNT * sizeof(float) / 3);
    res = res ? res : posix_memalign((void**)&in_2,       ALIGN_BYTES, WORD_COUNT * sizeof(float) / 3);
...
{
    return generic_get_fn(o, log, conv_get_3cf32_ci16_c, &last_fn_name);
}
...
static void print_data(const char* header)
{
    ...
    for(unsigned n = 0; n < 3; ++n)
    {
        ...
        {
            //fprintf(stderr, "%+.4f ", in[n][i]);
            fprintf(stderr, "%+2d ", (int16_t)(in[n][i] * 32767.f));
        }
    }
    ...
    {
        fprintf(stderr, "%+2d ", out[i]);
        //fprintf(stderr, "%+.4f ", (double)(out[i] / 32767.f));
    }
    fprintf(stderr, "\n");
}

START_TEST(conv_3cf32_ci16_check_simd)
{
...
START_TEST(conv_3cf32_ci16_speed)
{
    generic_opts_t opt = max_opt;
    conv_function_t fn = NULL;
    const void** pin = (const void**)in;
...
Suite * conv_3cf32_ci16_suite(void)
{
    max_opt = cpu_vcap_get();

    Suite* s = suite_create("conv_3cf32_ci16");

    ADD_REGRESS_TEST(s, conv_3cf32_ci16_check_simd);
    ADD_PERF_LOOP_TEST(s, conv_3cf32_ci16_speed, 60, 0, 3);
