...
static void setup()
{
    ...
    res = res ? res : posix_memalign((void**)&in_1,       ALIGN_BYTES, PACKET_SIZE * sizeof(int16_t) / 2);
...
{
    return generic_get_fn(o, log, conv_get_2ci16_ci12_c, &last_fn_name);
}
...
Suite * conv_2ci16_ci12_suite(void)
{
    ...
    Suite* s = suite_create("conv_2ci16_ci12");

    ADD_REGRESS_TEST(s, conv_2ci16_ci12_check_simd);
    ADD_PERF_LOOP_TEST(s, conv_2ci16_ci12_speed, 60, 0, 3);
